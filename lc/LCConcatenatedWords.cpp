...
class Trie {
    ...
    void insert(string word) {
        if (children[word[0] - 'a'] == NULL)
            children[word[0] - 'a'] = new Trie(word.size() <= 1);
        Trie *ptr = children[word[0] - 'a'];
        for (int i = 1; i < word.size(); i++) {
            if (ptr->children[word[i] - 'a'] == NULL)
                ptr->children[word[i] - 'a'] = new Trie((i + 1) == word.size());
            if ((i + 1) == word.size())
                ptr->children[word[i] - 'a']->isWord = true;
    ...
    vector<string> traverse(Trie *node) {
        vector<string> ans;
        if (node) {
            if (node->isWord) {
                ans.push_back("");
            ...
            for (int i = 0; i < node->children.size(); i++) {
                vector<string> res = traverse(node->children[i]);
                char c = 'a' + i;
                for (int j = 0; j < res.size(); j++) {
                    ans.push_back(c + res[j]);
                }
            }
        }
        return ans;
    ...
    vector<string> traverse() {
        ...
        for (int i = 0; i < children.size(); i++) {
            vector<string> res = traverse(children[i]);
            char c = 'a' + i;
            for (int j = 0; j < res.size(); j++) {
                ans.push_back(c + res[j]);
            }
    ...
    int getPrefix(string word, int idx) {
        if (idx >= word.size()) {
            return -1;
        }
        Trie *ptr = children[word[idx] - 'a'];
        ...
        return ptr ? word.size() - 1 : -1;
    }
    bool isCompositeWord(string word) {
        int p = getPrefix(word, 0) + 1;
        if (p == word.size()) {
            return false;
        }
        while (p < word.size() && p > 0) {
            p = getPrefix(word, p) + 1;
        }
        return p == word.size();
    }
    bool isCompositeWord(string &word, int idx) {
        if (idx >= word.size()) {
            return false;
        }
        cout << word << "\t" << idx << "\n";
        ...
        for (int i = idx + 1; i < word.size(); i++) {
            ...
            if (ptr->isWord) {
                // if (idx > 0 && i + 1 >= word.size()) {
                //     cout << "@\n";
                //     return true;
                // }
                if (isCompositeWord(word, i)) {
                    cout << "#\n";
                    return true;
                };
            }
        ...
        cout << "$\n";
        return idx > 0 && ptr ? ptr->isWord : false;
    }
};
class Solution {
    public:
    vector<string> findAllConcatenatedWordsInADict(vector<string> &words) {
        ...
        vector<string> ans;
        for (int i = 0; i < words.size(); i++) {
            if (trie->isCompositeWord(words[i], 0)) {
...
int main(int argc, char const *argv[]) {
    ...
    vector<string> ans = solution.findAllConcatenatedWordsInADict(strs);
